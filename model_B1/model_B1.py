...
class model_B1():
    def __init__(self, referenceset_x, referenceset_y,
                 ...
                 nca_model_path = "./Count_NCA.joblib",
                 knn_model_path = "./Count_Model.joblib"):
    ...
    def encode(self, message_text):
        word_counts = self.count_vectorizer.transform([message_text]).toarray()
        nca_message = self.nca_model.transform(word_counts)[0]
...
if __name__ == "__main__":
    referenceset_x = np.load("./Count_ReferenceSet_X.npy") # Array with message encodings
    referenceset_y = np.load("./Count_ReferenceSet_Y.npy") # Array with message categories

    model = model_B1(referenceset_x, referenceset_y)
